...
const std::string Base64ProtocolDecorator::headerTemplate("$V1;%s;%s;%x;T;%x###%s"); // localName, toName, messageType, messageSize, message

std::vector<std::uint8_t> Base64ProtocolDecorator::formMessage(const std::string& to, command t, std::size_t sz, const std::uint8_t* data) const {
	auto res = formMessage(to, t, ProtocolUtils::base64_encode(data, sz));
	return std::vector<std::uint8_t>(res.cbegin(), res.cend());
}

std::string Base64ProtocolDecorator::formMessage(const std::string& to, command t, const std::string& data) const {
	return (boost::format(headerTemplate)%localName%to%t%data.size()%data).str();
}

void Base64ProtocolDecorator::parseMessage(const message_ptr m) {
